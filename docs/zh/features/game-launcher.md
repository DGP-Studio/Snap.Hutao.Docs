...
胡桃工具箱的“资源下载”页签提供“中国大陆服务器”的游戏安装包和增量包的实时链接，用户可直接跳转至浏览器下载。
...
1. **定位备份文件**：
   - 胡桃工具箱会在 `%Your_Profile_Path%/ServerCache` 目录下存储转换前的游戏备份文件。
...
将配置文件保存到 `%Your_Launcher_Folder%/Genshin Impact Game/` 目录即可恢复正常启动。
