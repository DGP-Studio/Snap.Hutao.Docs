...
如果你使用了网络代理，请务必检查是否[解除了 Windows 容器 Loopback 限制](FAQ.md#如何通过网络代理使用胡桃工具箱)（注：`SteamCommunity302` 等加速器软件也需要注意此问题）
...
由于网络波动或者特殊网络的防火墙配置阻断了胡桃的数据所致，请尝试将校园网更换为手机热点或其他可信网络。

### 响应内容不是有效的文件字节流

因特殊网络（如校园网）所致，请尝试更换网络。
