...
如果你使用了网络代理，请务必检查是否[解除了 Windows 容器 Loopback 限制](FAQ.md#如何通过网络代理使用胡桃工具箱)（注：其他代理网络的加速器软件也需要注意此问题）
...
原因不明，可能由于网络波动或 DNS 等诸多方面有关，您可以尝试尝试切换网络（例如将校园网更换为其他网络），或尝试更换 DNS 等。
