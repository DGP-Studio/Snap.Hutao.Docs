...
| **Рекомендуемая версия ОС** | Windows 11 Build 22621 (22H2) или более поздняя версия           |
| **Необходимые компоненты**  | Windows Update, App Installer                                    |
| Дополнительные компоненты   | WebView2 Runtime, **MSVC Runtime Library**                       |

**Неподдерживаемые версии ОС**:

1.  **Версии LTSC и LTSB** не поддерживаются напрямую из-за отсутствия достаточно новой версии MSIX Core, требуется ручная обработка.
2.  **Не поддерживаются корпоративные и облегченные версии системы, все проблемы из неофициальных полных версий системы Microsoft не будут активно обрабатываться**.
3.  Windows 10 Build 19041 также может быть установлен, но некоторые функции не будут работать из-за проблем с ядром, поэтому проблемы Snap Hutao в этой версии не будут обрабатываться.
4.  Проблемы в предварительных версиях Windows Insider не будут обрабатываться.
